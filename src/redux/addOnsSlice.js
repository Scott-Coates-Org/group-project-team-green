...
async function _fetchAlladdOnsFromDb() {
  constaddOntSnapshot = await firebaseClient.firestore().collection("addOns").get()
...
export const addNewAddOn = createAsyncThunk(
  ...
  async (payload) => {
    try {
      await _addNewAddOn(payload.addOnName, payload.addOnPrice, payload.addOnPhoto)
      return payload
    } catch (error) {
...
export const savePhoto = createAsyncThunk("addOns/savePhoto", async (payload) => {
